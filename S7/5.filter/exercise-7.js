...
const formFilter = document.querySelector('input[data-function="toFilterStreamers"]')

formFilter.addEventListener("input", function () {
    const filteredStreamers = streamers.filter(guy => guy.name.toLowerCase().includes(formFilter.value.toLowerCase())
    );
